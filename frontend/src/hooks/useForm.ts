...
export default function useForm<Schema extends ZodSchema, Fields extends Keys<Schema>, FormElement extends BasicForm<Schema>, IFormValue extends z.infer<Schema>>(
  ...
  const setFiledValue = <Key extends Fields = Fields, Value extends IFormValue[Key] | undefined = IFormValue[Key] | undefined, Updater extends (curr?: Value) => Value = (curr?: Value) => Value>(
    key: Key,
    value: Value | Updater
  ) => {
    setFormData((curr) => {
  ...
  }, [formElements, formData, errors]); // eslint-disable-line
